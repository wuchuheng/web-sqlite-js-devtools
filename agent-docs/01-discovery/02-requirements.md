...
| FR-009   | Route `/openedDB/<db-name>` redirects to `/openedDB/<db-name>/tables`     | Clicking database navigates to Tables tab (default)                                |
| FR-009.1 | Database-level tab navigation (Tables/Query/Migration/Seed/About)         | Tab header shows 5 tabs, clicking tab changes nested route                         |
| FR-009.2 | Nested route structure under `/openedDB/:dbname`                          | Each tab has separate route: /tables, /query, /migration, /seed, /about            |
...
| FR-015   | Database detail component with 5 tabs                                     | Tables, Query, Migration, Seed, About tabs with icons (Log is separate route)      |
| FR-016   | Table tab: List all tables in alphabetical order                          | Left column (25% width) shows sorted table list from PRAGMA queries                |
| FR-017   | Table tab: Route `/openedDB/:dbname/tables/:tableName` for selected table | Clicking table updates nested route and displays active state styling              |
| FR-018   | Table tab: Multi-table tab bar (10% height)                               | Each table opens in separate tab with icon, name, and close button                 |
| FR-019   | Table tab: Table content area (75% width, split view)                     | Displays paginated table data (left) + DDL panel (right) when table selected       |
...
| FR-022   | Table tab: DDL info panel (right column of split view, always visible)    | Right column displays CREATE TABLE, indexes, triggers, views when table selected   |
